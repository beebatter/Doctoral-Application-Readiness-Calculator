...
export default function PhDEvalCalculator() {
  ...
  const weights = useMemo(() => {
    ...
    if (scheme === "engineering") return { AG: 0.25, RP: 0.25, RE: 0.25, RL: 0.15, PI: 0.10 };
    return { AG: 0.25, RP: 0.30, RE: 0.20, RL: 0.15, PI: 0.10 };
  }, [scheme]);
  ...
  const ts = useMemo(() => {
    ...
    return clamp(weights.AG * agPrime + weights.RP * rpPrime + weights.RE * rePrime + weights.RL * rlPrime + weights.PI * pi + delta, 0, 10);
  }, [agPrime, rpPrime, rePrime, rlPrime, pi, delta, meetsEnglish, weights]);

  const verdict = useMemo(() => {
    if (!meetsEnglish) return { label: "未达英语门槛", color: "bg-rose-500" };
    if (ts >= 8.5) return { label: "竞争力极强", color: "bg-emerald-600" };
    if (ts >= 7.5) return { label: "具有竞争力", color: "bg-amber-500" };
    if (ts >= 7.0) return { label: "边缘/看匹配", color: "bg-yellow-500" };
  ...
  const improvementHints = useMemo(() => {
    ...
    const items = [
      { key: "AG'", w: weights.AG, val: agPrime, tip: "提高学术成绩（优先完成硕士高分或补强数学/核心课）" },
  ...
  }, [meetsEnglish, agPrime, rpPrime, rePrime, rlPrime, pi, weights]);

  const weightedBreakdown = useMemo(() => {
    return [
      { name: "AG' 学术成绩", w: weights.AG, v: agPrime },
  ...
  }, [agPrime, rpPrime, rePrime, rlPrime, pi, weights]);
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-white text-gray-900">
      <div className="mx-auto max-w-6xl px-4 py-8 md:py-12">
        <header className="mb-6 md:mb-8">
          ...
          <p className="text-gray-600 mt-2">量纲统一、可解释的自评工具。EF 为门槛；TS≥8.5 竞争力极强，7.5–8.4 具有竞争力，7.0–7.4 边缘；&lt;7 建议补强。</p>
        </header>
        ...
        <Section title="A. 学术成绩 AG′（0–10）" subtitle="若本科与硕士皆有，默认 AG′ = 0.4·UG + 0.6·PG（自动计算）">
          ...
          <div className="mt-4 text-sm text-gray-700">自动计算 AG′ = {ugScore !== undefined && pgScore !== undefined ? `0.4×${ugScore.toFixed(2)} + 0.6×${pgScore.toFixed(2)} = ` : ""}<span className="font-semibold">{agPrime.toFixed(2)}</span></div>
        </Section>
